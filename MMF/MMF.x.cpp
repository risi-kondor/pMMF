...
MMF::MMF(const MMF& x) :
		cumulativeError(x.cumulativeError), finalA(
				new BlockedCmatrix(*x.finalA)) {
...
MMF::MMF(MMF&& x) :
		cumulativeError(x.cumulativeError), finalA(x.finalA) {
#ifdef _MMFCOPYWARNING
...
MMF& MMF::operator=(const MMF& x) {
	...
	delete finalA;
	finalA = new BlockedCmatrix(*x.finalA);
	for (auto p : stage)
		delete p;
	stage.clear();
	for (auto p : x.stage)
...
MMF MMF::collapse() {
	...
	x.finalA = new BlockedCmatrix(*finalA);
...
BlockedCmatrix MMF::reconstruction() const {
	...
	BlockedCmatrix* A = const_cast<BlockedCmatrix*>(finalA);
...
void MMF::invert(const FIELD eps) {
	...
	if (eps != 0) {
		Cmatrix& A = *finalA->block[0];
		for (int i = 0; i < A.nrows; i++)
			A(i, i) += eps;
		replace(finalA, new BlockedMatrix<Cmatrix>(finalA->inverse()));
	} else
		replace(finalA, new BlockedMatrix<Cmatrix>(finalA->inverse()));
...
void MMF::invertSqrt(const FIELD eps) {
	...
	if (eps != 0) {
		...
		for (int i = 0; i < A.nrows; i++)
			A(i, i) += eps;
		replace(finalA, new BlockedMatrix<Cmatrix>(finalA->inverseSqrt()));
	} else
		replace(finalA, new BlockedMatrix<Cmatrix>(finalA->inverseSqrt()));
